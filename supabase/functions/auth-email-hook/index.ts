...
const EMAIL_SUBJECTS: Record<string, string> = {
  signup: 'Confirm your email · Google Review AI',
  invite: "You've been invited · Google Review AI",
  magiclink: 'Your Google Review AI login link',
  recovery: 'Reset your Google Review AI password',
  email_change: 'Confirm your new email · Google Review AI',
  reauthentication: 'Your Google Review AI verification code',
}
...
const SITE_NAME = 'Google Review AI'
// Keep verified technical domains until notify.googlereviewai.com is verified with the email provider.
const SENDER_DOMAIN = 'notify.ranki.ai'
const ROOT_DOMAIN = 'googlereviewai.com'
const FROM_DOMAIN = 'ranki.ai'

const SAMPLE_PROJECT_URL = 'https://googlereviewai.com'
...
async function handlePreview(req: Request): Promise<Response> {
  ...
  let type: string
  try { type = (await req.json()).type }
  catch { return new Response(JSON.stringify({ error: 'Invalid JSON in request body' }), { status: 400, headers: { ...previewCorsHeaders, 'Content-Type': 'application/json' } }) }
...
async function handleWebhook(req: Request): Promise<Response> {
  ...
  const { error: enqueueError } = await supabase.rpc('enqueue_email', {
    ...
    payload: {
      ...
      subject: EMAIL_SUBJECTS[emailType] || 'Google Review AI notification',
      html,
      text,
