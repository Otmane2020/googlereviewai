            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                <div className="truncate text-sm font-semibold">
                  {direction === "inbound" ? message.from_email : (message.to_emails?.join(", ") || "—")}
                </div>
                <div className="shrink-0 text-xs text-muted-foreground">{formatDate(message.created_at)}</div>
              </div>
              <div className="mt-1 flex items-center gap-2">
                <div className="truncate text-sm text-foreground">{message.subject || "(sans objet)"}</div>
                <Badge variant="outline" className="shrink-0 text-[10px]">{message.status}</Badge>
              </div>
              ...
              {isOpen && <div className="mt-4 rounded-lg bg-muted/40 p-4 text-sm leading-6 text-foreground">
                <div className="mb-3 space-y-1 text-xs text-muted-foreground">
                  ...
                  <div><strong>À :</strong> {message.to_emails?.join(", ") || "—"}</div>
                  {!!message.cc_emails?.length && <div><strong>CC :</strong> {message.cc_emails.join(", ")}</div>}
                  {message.source && <div><strong>Source :</strong> {message.source}</div>}
                </div>
  ...
  return (
    <Card className="mb-6">
      <CardHeader className="flex flex-row items-center justify-between gap-3">
        <div>
          <CardTitle className="text-lg">Emails Google Review AI</CardTitle>
          <p className="mt-1 text-sm text-muted-foreground">Boîte de réception et historique des emails envoyés.</p>
        </div>
        <Button variant="outline" size="sm" onClick={() => void loadMessages()} disabled={loading} className="gap-2">
      ...
      <CardContent>
        <Tabs defaultValue="inbox">
          <TabsList className="mb-4">
            <TabsTrigger value="inbox" className="gap-2"><Inbox className="h-4 w-4" /> Réception <Badge variant="secondary">{inbox.length}</Badge></TabsTrigger>
          ...
          <TabsContent value="inbox">{renderRows(inbox, "inbound")}</TabsContent>
          <TabsContent value="sent">{renderRows(sent, "outbound")}</TabsContent>
        </Tabs>
      </CardContent>
    </Card>
  );
