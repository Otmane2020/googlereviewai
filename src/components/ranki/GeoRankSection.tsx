...
const getQueries = (isFrench: boolean) => [
  {
    engine: "ChatGPT",
    query: isFrench ? "Meilleur restaurant italien à Paris avec terrasse ?" : "Best Italian restaurant in Paris with a terrace?",
    your: { name: "Trattoria Romana", rank: 1, change: "+4" },
    competitors: ["Cucina Lucia", "Forno Vecchio", "Pasta Bar 11"],
  },
  {
    engine: "Perplexity",
    query: isFrench ? "Meilleurs hôtels boutique à Lyon pour un week-end ?" : "Best boutique hotels in Lyon for a weekend?",
    your: { name: "Maison Driskill", rank: 2, change: "+1" },
    competitors: ["Hôtel Sainte-Cécile", "South Hôtel"],
  },
  {
    engine: "Gemini",
    query: isFrench ? "Garage automobile pas cher à Marseille ?" : "Affordable auto repair shop in Marseille?",
...
export const GeoRankSection = () => {
  ...
  const queries = getQueries(i18n.language?.startsWith("fr"));
  return (
